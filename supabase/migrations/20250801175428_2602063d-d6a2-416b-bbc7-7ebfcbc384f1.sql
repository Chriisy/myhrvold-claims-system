-- Create trigger function to automatically log all claim changes to timeline
...
-- Create trigger to automatically log all claim changes
DROP TRIGGER IF EXISTS trigger_log_claim_changes ON public.claims;
CREATE TRIGGER trigger_log_claim_changes
  AFTER INSERT OR UPDATE ON public.claims
  FOR EACH ROW
  EXECUTE FUNCTION public.log_claim_changes();

-- Update existing timeline entries to use Norwegian action descriptions
UPDATE public.claim_timeline 
SET notes = CASE 
  WHEN notes = 'Claim created' THEN 'Reklamasjon opprettet'
  WHEN notes = 'Status updated' THEN 'Status oppdatert'
  WHEN notes = 'Claim updated' THEN 'Reklamasjon oppdatert'
  ELSE notes
END
WHERE notes IN ('Claim created', 'Status updated', 'Claim updated');