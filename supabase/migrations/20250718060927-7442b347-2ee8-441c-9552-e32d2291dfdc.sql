-- First, let me check if we have any users in profiles
DO $$
DECLARE
    user_id uuid;
BEGIN
    -- Get first technician user
    SELECT id INTO user_id FROM profiles WHERE role = 'technician' LIMIT 1;
    
    IF user_id IS NULL THEN
        RAISE EXCEPTION 'No users found in profiles table. Please create a user first.';
    END IF;
    
    -- Insert 10 test claims with different brands and complete data
